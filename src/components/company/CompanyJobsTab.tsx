  ...
  if (jobs.length === 0) {
    return (
      <div className="text-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
        ...
        <h3 className="text-xl font-bold text-slate-900 mb-2">Chưa có tin tuyển dụng nào</h3>
        <p className="text-slate-500">Công ty chưa đăng tải tin tuyển dụng nào.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h3 className="text-xl font-bold text-slate-800">Tin tuyển dụng</h3>
          <p className="text-sm text-slate-500 mt-1">Tổng cộng {jobs.length} vị trí đang tuyển</p>
        </div>
                    ...
                    <div className="flex-1 space-y-3 min-w-0">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <h4 className="font-bold text-lg mb-2 text-slate-900 group-hover:text-[var(--brand)] transition-colors truncate">
                            {job.title}
                          </h4>
                        ...
                        {companyLogoUrl && (
                          <div className="w-12 h-12 relative bg-white rounded-lg border border-slate-100 p-1 flex-shrink-0">
              ...
              <Card className="hover:border-[var(--brand)]/50 transition-colors h-full">
                <div className="p-6 space-y-4">
                  <div className="flex items-start justify-between gap-2">
                    <h4 className="font-bold text-lg text-slate-900 group-hover:text-[var(--brand)] transition-colors line-clamp-2 flex-1">
                      {job.title}
                    </h4>
                    {companyLogoUrl && (
                      <div className="w-10 h-10 relative bg-white rounded-lg border border-slate-100 p-1 flex-shrink-0">
                        <Image src={companyLogoUrl} alt={companyName} fill className="object-contain p-1" />
                      </div>
                    )}
                  </div>
