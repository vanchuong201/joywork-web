...
/**
 * Chế độ SEO landing: URL không mang query string, bộ lọc đến từ mapping đã
 * publish. Mọi thay đổi bộ lọc của người dùng điều hướng về `/jobs?...` để
 * URL SEO không bị biến dạng.
 */
export type JobsListingSeoMode = {
  /** Query string chuẩn hóa của mapping, ví dụ `location=ha-noi&q=Marketing`. */
  search: string;
  heading: string;
  initialJobs?: Job[];
  initialPagination?: JobsPagination;
};

type JobsListingPageClientProps = {
  seoMode?: JobsListingSeoMode;
};
...
function JobsPageContent({
  ...
  /** Bộ lọc hiện hành. Ở SEO landing lấy từ mapping, không từ URL. */
  sp: URLSearchParams;
  seoMode?: JobsListingSeoMode;
}) {
  const routePathname = usePathname();
  const router = useRouter();
  const pathname = seoMode ? "/jobs" : routePathname;
  ...
  // Sync viewMode to URL — bỏ qua ở SEO landing để không gắn query vào slug
  useEffect(() => {
    if (seoMode) return;
  ...
  const { data, isLoading, isFetching } = useQuery<{ jobs: Job[]; pagination: JobsPagination }>({
    ...
    // Hydrate từ dữ liệu SSR của landing để crawler thấy danh sách ngay trong HTML
    initialData:
      seoMode?.initialJobs && page === 1
        ? { jobs: seoMode.initialJobs, pagination: seoMode.initialPagination ?? {} }
        : undefined,
  });

  const { data: featuredData, isLoading: featuredLoading, isFetching: featuredFetching } = useQuery<{ jobs: Job[]; pagination: JobsPagination }>({
    ...
    <div className="space-y-8">
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-semibold">{seoMode?.heading ?? "Tìm kiếm việc làm"}</h1>
            {companyId ? (
      ...
      <TooltipProvider delayDuration={300}>
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold">Khám phá công ty nổi bật</h2>
...
/** Trang `/jobs`: bộ lọc đọc từ query string, cần Suspense cho `useSearchParams`. */
function JobsListingWithRouteParams() {
  const sp = useSearchParams();
  return <JobsPageContent sp={sp} />;
}

/**
 * SEO landing: bộ lọc đến từ mapping nên không dùng `useSearchParams`, nhờ đó
 * danh sách việc làm được render ngay trong HTML trả về cho crawler.
 */
function JobsListingSeoContent({ seoMode }: { seoMode: JobsListingSeoMode }) {
  const sp = useMemo(() => new URLSearchParams(seoMode.search), [seoMode.search]);
  return <JobsPageContent sp={sp} seoMode={seoMode} />;
}

export default function JobsListingPageClient({ seoMode }: JobsListingPageClientProps) {
  if (seoMode) {
    return <JobsListingSeoContent seoMode={seoMode} />;
  }
